...
if __name__ == "__main__":
  wlg = WorkloadGenerator()
  
  # Different number generators fro testing
  num_ops = [1000, 5000, 20000, 80000]
  dss = ["BST", "List", "Heap", "Hash", "Array"]
  workloads = [wlg.workload_A, wlg.workload_B, wlg.workload_C, wlg.workload_D]
  ...
  for ds in dss:
    for workload in workloads:
      for num in num_ops:
        ...
        # safe file to [ds]_[workload]_[num].txt
        # maybe include timestamp
        filename = f"{ds}_{workload.__name__}_{num}_{int(time.time())}.txt"